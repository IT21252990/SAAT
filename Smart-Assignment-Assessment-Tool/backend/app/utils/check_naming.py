...
#     """
#     Use Gemini API to check file naming conventions.
#     Returns a JSON object with the analysis results.
#     """
#     prompt = f"""
#     You are an AI Tutor Analyzing GitHub Repositories. Based on repository content at {RepoURL}.
#     Check if all the files in the repository adhere to standard naming conventions:
#     - File names should use lowercase letters with words separated by `_` (snake_case) or `-` (kebab-case).
#     - No spaces or special characters except `_` and `-`.
#     - File extensions must be appropriate and match common formats (e.g., `.py`, `.js`, `.html`, `.csv`, `.txt`, `.md`, `.json`, `.xml`, etc.).
#     - No duplicate file names (regardless of case sensitivity).
#     - Hidden files (starting with `.`) should only include system-related files like `.gitignore`, `.env`, etc.
#     Ignore setup folders like node-modules,.venv and auto generated directories

#     IMPORTANT: Return ONLY a valid JSON object with no additional text or formatting, exactly like this:
#     {{"status": "Yes"}} // If all files follow naming conventions
#     OR
#     {{"status": "No", "invalid_files": [{{"file_name": "filename", "path": "relative_path", "reason": "reason"}}, ...]}} // If errors found
#     """
...
#         return {"error": f"Failed to parse response: {str(e)}", "raw_response": response.text}
