...
// import ContributorBarChart from "../components"
...
//       <ContributorBarChart contributors={contributors} />
...
const ContributorCommitHistory = ({ repoUrl }) => {
  ...
  return (
    <div className="max-w-full mx-auto p-5 font-sans">
      <h2 className="text-2xl font-bold text-gray-800 dark:text-white mb-4">Contributors</h2>
      {error && <p className="text-red-600 mb-3">{error}</p>}
      {loading && (
        <p className="text-gray-500 italic mb-3">Loading contributors...</p>
      )}

<ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 w-full p-4 rounded-lg bg-white shadow dark:border dark:border-gray-700 dark:bg-gray-800">
  {contributors.map((contributor) => (
    <li
      key={contributor.id}
      className="flex items-center p-3 border border-gray-200 rounded-lg cursor-pointer transition-colors duration-300 hover:bg-gray-100 bg-gray-400"
      onClick={() => handleContributorClick(contributor)}
    >
      <img
        src={contributor.avatar_url}
