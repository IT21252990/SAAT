...
const ReportUpload = ({ onSubmit }) => {
      ...
      reader.onload = async (event) => {
        try {
          ...
          for (let i = 1; i <= pdf.numPages; i++) {
            ...
            const pageText = textContent.items.map(item => item.str).join(' '); // page eke okkoma aragena wenama text ekk hadagnnwa
            fullText += pageText + '\n';
          }
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const result = await analyzeReport( //call AI open service
